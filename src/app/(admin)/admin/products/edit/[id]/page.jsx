...
export default function AddProductPage({ params }) {
    ...
    useEffect(() => {
        fetch(`/api/products/${id}`).then(res => res.json()).then((data) => {
            const { title, price, images, isActive, sizes, stock, category, colors, description, brand } = data.product;
            console.log(data)
            setFormData({ title, comparePrice: "0000", price, status: isActive, brand, sizes: sizes.join(), stock, category, description, color: colors })
            setImages(images)
        })
    }, [])
